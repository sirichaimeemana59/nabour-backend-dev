	...
	<div class="page-title">
		...
		<div class="breadcrumb-env">
			<ol class="breadcrumb bc-1" >
				<li>
					<a href="{!! url('/') !!}"><i class="fa-home"></i>{!!  __('messages.page_home') !!}</a>
				</li>
				<li class="active">
					<strong>{!!  __('messages.Officer.page_head') !!}</strong>
				</li>
			</ol>
		</div>
	</div>
	<a href="#" data-toggle="modal" data-target="#add-officer-modal" class="action-float-right create-invoice-btn btn btn-primary">+ เพิ่มผู้ดูแลระบบ </a>
	<div class="row">
		<div class="col-md-12">
			<div class="panel panel-default">
				<div class="panel-heading">
                    <h3 class="panel-title">ผู้ดูแลระบบภายใน Nabour</h3>
                </div>
				<div class="panel-body member-list-content">
					<div class="tab-pane active" id="member-list">
						<div id="member-list-content">
							@include('admin.officer-list')
						</div>
					</div>
				</div>
			</div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">{!!  __('messages.Member.confirm_ban_head') !!}</h4>
                </div>
                <div class="modal-body">
                	{!!  __('messages.Member.confirm_ban_msg') !!}
                </div>
                 <div class="modal-footer">
                    <button type="button" class="btn btn-white" data-dismiss="modal">{!!  __('messages.cancel') !!}</button>
                    <button type="submit" class="btn btn-primary change-active-status-btn">{!!  __('messages.confirm') !!}</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="modal-inactive" data-backdrop="static">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <h4 class="modal-title">{!!  __('messages.Member.confirm_active_head') !!}</h4>
                </div>
                <div class="modal-body">
                	{!!  __('messages.Member.confirm_active_msg') !!}
                </div>
                 <div class="modal-footer">
                    <button type="button" class="btn btn-white" data-dismiss="modal">{!!  __('messages.cancel') !!}</button>
                    <button type="submit" class="btn btn-primary change-active-status-btn">{!!  __('messages.confirm') !!}</button>
	...
	<div class="modal fade" id="modal-delete-member" data-backdrop="static">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-white" data-dismiss="modal">{!!  __('messages.cancel') !!}</button>
					<button type="submit" class="btn btn-primary delete-member-btn">{!!  __('messages.confirm') !!}</button>
				</div>
			</div>
		</div>
	</div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
				...
				<div id="form-add-officer">
					<div class="form">
						@include('admin.officer-form')
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-white" data-dismiss="modal">{!!  __('messages.cancel') !!}</button>
						<button type="button" class="btn btn-primary save-officer">{!!  __('messages.save') !!}</button>
					</div>
				</div>
            </div>
        </div>
...
<script type="text/javascript">
	$(function (){
        ...
        $('#form-add-officer .save-officer').on('click',function () {
			...
			$.ajax({
				url     : $('#root-url').val()+"/root/admin/admin-system/add",
                method	: "POST",
		...
		$('#form-edit-officer .save-officer').on('click',function () {
            ...
            btn.attr('disabled','disabled').prepend('<i class="fa-spin fa-spinner"></i> ');
			$.ajax({
				url     : $('#root-url').val()+"/root/admin/admin-system/edit",
                method	: "POST",
		...
		$('.member-list-content').on('click','.view-member', function (e){
			...
			$.ajax({
				url     : $('#root-url').val()+"/root/admin/admin-system/view",
                method	: "POST",
                data 	: ({uid:uid}),
                dataType: "html",
                success: function (t) {
		...
		$('.member-list-content').on('click','.edit-member', function (e){
			...
			var uid = _this.attr('data-uid');
			$.ajax({
				url     : $('#root-url').val()+"/root/admin/admin-system/edit/get",
                method	: "POST",
                data 	: ({uid:uid}),
                dataType: "html",
                success: function (t) {
		...
		function changeActiveStatus (uid,flag) {
			$.ajax({
				url     : $('#root-url').val()+"/root/admin/admin-system/active",
                method	: "POST",
		...
		function deleteMemberAccout(uid) {
			$.ajax({
				url     : $('#root-url').val()+"/root/admin/admin-system/delete",
				method	: "POST",
				data 	: ({uid:uid}),
				dataType: "json",
				success: function (r) {
